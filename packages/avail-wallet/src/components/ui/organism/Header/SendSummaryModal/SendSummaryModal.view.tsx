...
export const SendSummaryModalView = ({ address, amount, chainId, closeModal }: Props) => {
  ...
  useEffect(() => {
    const fetchGasFee = () => {
      if (wallet.accounts) {
        ...
        const amountBN = ethers.utils.parseUnits(amount, wallet.tokenBalance.decimals);
  ...
  useEffect(() => {
    if (gasFees?.suggestedMaxFee) {
      ...
      // }
      const amountBN = ethers.utils.parseUnits(amount, wallet.tokenBalance.decimals);
      // if (wallet.tokenBalance.address === ethToken.address) {
  ...
  useEffect(() => {
    ...
    // wallet.tokenBalance.usdPrice &&
    //   setAmountUsdPrice(getAmountPrice(wallet.tokenBalance, amountFloat, false));
  ...
  return (
    <div>
      <Wrapper>
        ...
        <AssetQuantity
          currency={wallet.tokenBalance.symbol}
          currencyValue={getMaxDecimalsReadable(wallet.tokenBalance, amount)}
          size="medium"
          centered
        />
        <Summary>
          <LeftSummary>
            <PopperTooltip
              ...
              content={
                <EstimatedFeesTooltip>
                  Gas fees are defined by the network and fluctuate depending on network traffic and
        ...
        {totalExceedsBalance && (
          <AlertTotalExceedsAmount text="Insufficient funds for fees" variant="warning" />
        )}
      </Wrapper>
      <Buttons>
