...
export const useAvailSnap = () => {
  ...
  const snapId = process.env.NEXT_PUBLIC_SNAP_ID || 'npm:@avail-project/avail-snap';
  ...
  const switchNetwork = async (network: number, chainId: string) => {
    ...
    try {
      ...
      const result = await provider.send('wallet_switchEthereumChain', [{ chainId }]);
  ...
  const getWalletData = async (networkId: number, updateAccounts: boolean) => {
    try {
      if (updateAccounts) {
        ...
        setAccounts(accounts.map((address: string) => ({ address, publicKey: '' })));

        // This is a placeholder - you'll need to implement actual balance fetching
        setErc20TokenBalanceSelected({ amount: '1000', symbol: 'AVAIL', decimals: 18 });
      }
    } catch (error) {
